...
- **SwiftUI-first:** All UI is declarative, using custom components (cards, badges, steppers) for a premium feel.
- **CoreData:** Session logs and user data are persisted via CoreData (`SessionLog`, `PersistenceController`).
- **AppStorage:** User settings (durations, feedback toggles, goals) use `@AppStorage` for instant sync.
- **Singletons:** `DataManager` and `TimerManager` are singletons for global state and logic.
...
- **Animations:** Spring and ease animations are used for transitions, steppers, and card reveals. Avoid excessive gradients; prefer neutral system colors.
